...
def init_db():
    ...
    try:
        ...
        # --- NEW IN V72: INVENTORY SYSTEM ---
        # 5. Master Inventory (Gudang Utama)
        try: ws_m = sh.worksheet("Master_Inventory")
        except: ws_m = sh.add_worksheet("Master_Inventory", 1000, 10)
        h_m = ["ItemCode", "ItemName", "Category", "CostPrice", "SellPrice", "CurrentStock", "WarrantyDays", "Supplier", "LastUpdated"]
        if ws_m.row_values(1) != h_m: ws_m.update("A1:I1", [h_m])

        # 6. Restock Log (Rekod Beli Barang)
        try: ws_r = sh.worksheet("Restock_Log")
        except: ws_r = sh.add_worksheet("Restock_Log", 1000, 10)
        h_r = ["LogID", "Date", "InvoiceNo", "Supplier", "ItemCode", "ItemName", "QtyAdded", "CostPrice", "TotalCost"]
        if ws_r.row_values(1) != h_r: ws_r.update("A1:I1", [h_r])
        # ------------------------------------
...
def load_data(tab_name):
    ...
    try:
        ...
        # Patch Empty
        if df.empty:
            if tab_name == "Tickets": df = pd.DataFrame(columns=["ID", "Tarikh", "Customer", "Phone", "Email", "Model", "SN", "Password", "Masalah", "Fizikal", "Aksesori", "Status", "Kos_Part", "Harga_Jual", "Image_Link", "Tech_Note"])
            elif tab_name == "Master_Inventory": df = pd.DataFrame(columns=["ItemCode", "ItemName", "Category", "CostPrice", "SellPrice", "CurrentStock", "WarrantyDays", "Supplier", "LastUpdated"])
...
def update_stock(item_code, qty_change):
    # qty_change: Positive to add, Negative to deduct
    client = get_client(); sheet = client.open_by_key(SHEET_ID).worksheet("Master_Inventory")
    try:
        cell = sheet.find(str(item_code))
        if cell:
            curr = safe_int(sheet.cell(cell.row, 6).value) # Col 6 is CurrentStock
            new_qty = curr + int(qty_change)
            sheet.update_cell(cell.row, 6, new_qty)
            st.cache_data.clear()
            return True
    except: pass
    return False
...
def update_customer_info_db(tid, nama, phone, email, model, sn, pwd, masalah):
    ...
    if cell:
        sheet.update_cell(cell.row, 3, nama); sheet.update_cell(cell.row, 4, phone)
        sheet.update_cell(cell.row, 5, email); sheet.update_cell(cell.row, 6, model)
        sheet.update_cell(cell.row, 7, sn); sheet.update_cell(cell.row, 8, pwd)
        sheet.update_cell(cell.row, 9, masalah); st.cache_data.clear(); return True
    return False
...
def update_part_data(part_id, new_name, new_supp, new_price, new_warranty):
    ...
    if cell:
        sheet.update_cell(cell.row, 3, new_name); sheet.update_cell(cell.row, 4, new_supp)
        sheet.update_cell(cell.row, 6, new_warranty)
        new_exp = (datetime.now() + pd.DateOffset(months=int(new_warranty))).strftime("%Y-%m-%d")
        sheet.update_cell(cell.row, 7, new_exp); sheet.update_cell(cell.row, 8, new_price)
        st.cache_data.clear(); return True
    return False
...
def draw_wrapped_text(c, text, x, y, max_width, font="Helvetica", size=10):
    c.setFont(font, size); text = str(text); words = text.split(); lines = []; current_line = []
    for word in words:
        test_line = ' '.join(current_line + [word])
        if c.stringWidth(test_line, font, size) < max_width: current_line.append(word)
        else: lines.append(' '.join(current_line)); current_line = [word]
    lines.append(' '.join(current_line))
    curr_y = y
    for line in lines: c.drawString(x, curr_y, line); curr_y -= (size + 2)
    return curr_y

def generate_pdf(t, type="SERVICE"):
    buffer = BytesIO(); p = canvas.Canvas(buffer, pagesize=A4); w, h = A4; cfg = st.session_state.config
    logo_url = cfg.get("Logo", "")
    if logo_url and len(logo_url) > 10:
        try: logo = ImageReader(logo_url); p.drawImage(logo, 40, h-100, width=80, height=80, mask='auto', preserveAspectRatio=True)
        except: pass
    p.setFont("Helvetica-Bold", 20); p.drawString(140, h-50, cfg.get("Company_Name", "DCK TECH SERVICES"))
    p.setFont("Helvetica", 10); p.drawString(140, h-65, cfg.get("Address", "Alamat Kedai")); p.drawString(140, h-78, f"Tel: {cfg.get('Phone', '')}")
    p.setLineWidth(1.5); p.line(40, h-110, w-40, h-110)
    p.setFont("Helvetica-Bold", 14); doc_title = "SERVICE TICKET" if type == "SERVICE" else "OFFICIAL RECEIPT"
    if type == "INVOICE": doc_title = "OFFICIAL INVOICE"
    p.drawCentredString(w/2, h-130, doc_title)
    p.setLineWidth(0.5); p.rect(40, h-220, w-80, 80)
    ...
    if 'Model' in t: 
        ...
        p.drawString(50, h-175, f"Name: {t.get('Customer', '-')}"); p.drawString(300, h-175, f"Receipt ID: {t.get('ID', '-')}"); p.drawString(300, h-190, f"Date: {t.get('Tarikh', '-')}")
    y = h-250; p.setFont("Helvetica-Bold", 11); p.setFillColorRGB(0.9, 0.9, 0.9); p.rect(40, y, w-80, 20, fill=1); p.setFillColorRGB(0, 0, 0)
    if 'Model' in t: 
        p.drawString(50, y+6, "DEVICE / MODEL"); p.drawString(250, y+6, "DIAGNOSIS / PROBLEM"); p.drawString(450, y+6, "REMARKS")
        y -= 25; y_model = draw_wrapped_text(p, t.get('Model', '-'), 50, y, 180); y_prob = draw_wrapped_text(p, t.get('Masalah', '-'), 250, y, 180)
        y_note = draw_wrapped_text(p, t.get('Tech_Note', '-'), 450, y, 100); y = min(y_model, y_prob, y_note) - 15
        p.line(40, y, w-40, y); y -= 20; p.setFont("Helvetica-Bold", 10); p.drawString(50, y, "Condition & Accessories:")
        ...
        y -= 25; p.setFont("Helvetica", 10); items = t.get('Items_List', [])
        if not items: items = [{'Item': t.get('Item'), 'Qty': t.get('Qty'), 'Harga_Unit': t.get('Harga_Unit')}]
        for itm in items:
            p.drawString(350, y, str(itm.get('Qty', 1))); p.drawString(450, y, f"RM {safe_float(itm.get('Harga_Unit', 0)):.2f}")
            y_item = draw_wrapped_text(p, str(itm.get('Item', '-')), 50, y, 280); y = y_item - 10
        p.line(40, y, w-40, y)
    if type in ["INVOICE", "SALES"] or t.get('Status') in ['Done', 'Collected']:
        total = safe_float(t.get('Harga_Jual', t.get('Total', 0))); y -= 30; p.setFont("Helvetica-Bold", 14); p.drawRightString(w-50, y, f"TOTAL: RM {total:.2f}")
    y_footer = 150; p.line(40, y_footer, w-40, y_footer); p.setFont("Helvetica-Bold", 9); p.drawString(40, y_footer-15, "TERMS & CONDITIONS:")
    p.setFont("Helvetica", 8); tnc_text = cfg.get("TNC_Invoice", "") if type in ["INVOICE", "SALES"] else cfg.get("TNC_Ticket", "")
    curr_y = y_footer - 30
    for line in tnc_text.split('\n'): p.drawString(40, curr_y, line.strip()); curr_y -= 12
    y_sig = 50; p.line(50, y_sig, 200, y_sig); p.line(350, y_sig, 500, y_sig); p.drawString(50, y_sig-15, "Customer Signature"); p.drawString(350, y_sig-15, "Technician / Manager")
    p.save(); buffer.seek(0); return buffer

# --- 5. EMAIL & LINKS ---
def generate_message_content(data):
    cust = str(data.get('Customer', 'Pelanggan')); status = str(data.get('Status', 'Pending')); tid = str(data.get('ID', '-'))
    model = str(data.get('Model', '-')); sn = str(data.get('SN', '-')); masalah = str(data.get('Masalah', '-'))
    note = str(data.get('Tech_Note', '-')); price = safe_float(data.get('Harga_Jual', 0))
    ...
    msg += f"\n\n--- BUTIRAN ---\nID: {tid}\nModel: {model}\nS/N: {sn}\nMasalah: {masalah}\nSolution: {note}"
    if status in ['Done', 'Collected']: msg += f"\n\n💰 TOTAL: RM {price:.2f}"; msg += "\n\nSekian,\nTeam DCK Tech"
    return msg

def generate_links(type, data):
    phone = clean_phone_number_my(data.get('Phone', '')); full_msg = generate_message_content(data)
    if type == "WA": wa_msg = full_msg.replace("BUTIRAN:", "*BUTIRAN:*"); return f"https://wa.me/{phone}?text={urllib.parse.quote(wa_msg)}"
    elif type == "EMAIL": sub = f"Tiket DCK: {data.get('ID', '-')} - {data.get('Model', '-')}"; return f"mailto:{data.get('Email', '')}?subject={urllib.parse.quote(sub)}&body={urllib.parse.quote(full_msg)}"
...
def send_email_with_pdf(to_email, data, pdf_buffer, pdf_name):
    ...
    part = MIMEApplication(pdf_buffer.getvalue(), Name=pdf_name); part['Content-Disposition'] = f'attachment; filename="{pdf_name}"'; msg.attach(part)
    try: s = smtplib.SMTP('smtp.gmail.com', 587); s.starttls(); s.login(sender, password); s.send_message(msg); s.quit(); return True, "Berjaya!"
    except Exception as e: return False, str(e)

# =============================================================================
# 🚦 V70 GATEKEEPER LOGIC: DIGITAL HEALTH CARD
# =============================================================================
query_params = st.query_params; sn_query = query_params.get("sn", None)
if sn_query:
    ...
    load_config(); cfg = st.session_state.config
    c_logo, c_title = st.columns([1, 4])
    with c_title: st.title(f"💻 {cfg.get('Company_Name', 'DCK TECH')} - Digital Profile"); st.caption("Rekod Profil & Sejarah Peranti")
    st.divider()
    with st.spinner(f"🔍 Memuatkan profil S/N: {sn_query}..."): df = load_data("Tickets")
    found = False
    if not df.empty:
        history = df[df['SN'].astype(str).str.strip().str.upper() == str(sn_query).strip().upper()]
        if not history.empty:
            found = True; latest = history.iloc[-1]
            st.success("✅ Peranti Sah & Berdaftar")
            with st.container(border=True):
                c1, c2 = st.columns(2); c1.write(f"**Model:** {latest.get('Model')}"); c2.write(f"**S/N:** {latest.get('SN')}")
                st.markdown("---"); st.write("**Sejarah Isu & Pembaikan:**"); st.info(f"{latest.get('Masalah')}")
                st.markdown(f"""<div style="background-color: #d4edda; padding: 10px; border-radius: 5px; border: 1px solid #c3e6cb; color: #155724;"><strong>✅ Solution / Tindakan:</strong><br>{latest.get('Tech_Note', '-')}</div>""", unsafe_allow_html=True)
            if len(history) > 1:
                st.write("### 📜 Sejarah Terdahulu (Klik untuk lihat Solution)"); prev_jobs = history.iloc[:-1].iloc[::-1]
                for _, row in prev_jobs.iterrows():
                    with st.expander(f"📅 {row['Tarikh']} : {row['Masalah']}"):
                        st.markdown(f"""<div style="background-color: #e2e3e5; padding: 10px; border-radius: 5px; color: #383d41;"><strong>✅ Solution:</strong><br>{row['Tech_Note']}</div>""", unsafe_allow_html=True)
            st.caption(f"Profil ini dijana automatik oleh sistem {cfg.get('Company_Name')}")
    if not found: st.error(f"❌ Maaf, tiada profil dijumpai untuk S/N: {sn_query}"); st.warning("Sila pastikan Serial Number dimasukkan dengan betul.")
    st.stop()
...
if st.session_state.page == "📊 DASHBOARD":
    ...
    today_str = datetime.now().strftime("%Y-%m-%d"); sales_today = 0.0
    ...
    st.write("### 🔍 Cari Ticket"); search = st.text_input("Masukkan Nama / ID / Model:", placeholder="Contoh: DCK-12345")
...
# === PAGE: DAFTAR TIKET ===
    ...
    with st.form("reg_ticket", clear_on_submit=True):
        ...
        with c1: st.markdown("### 👤 Pelanggan"); nama = st.text_input("Nama"); phone = st.text_input("No HP"); email = st.text_input("Email")
        with c2: st.markdown("### 💻 Peranti"); model = st.text_input("Model"); sn = st.text_input("Serial No"); pwd = st.text_input("Password")
        with c3: st.markdown("### 🔧 Diagnosis"); mslh = st.multiselect("Masalah", opt_mslh); fiz = st.multiselect("Fizikal", opt_fiz); acc = st.multiselect("Aksesori", opt_acc)
        note = st.text_area("Nota Tambahan"); img = st.file_uploader("Gambar Peranti", accept_multiple_files=True); tnc = st.checkbox("Saya setuju dengan Terma & Syarat")
        submitted = st.form_submit_button("SIMPAN REKOD", use_container_width=True)
        if submitted and nama and tnc:
            with st.spinner("Menyimpan..."):
                tid = f"DCK-{datetime.now().strftime('%d%H%M')}"; img_str = ""
                if img:
                ...
                add_row("Tickets", row); st.toast("Tiket Berjaya Dibuka!", icon='✅')
    ...
    if 'last_data' in st.session_state:
        ld = st.session_state.last_data; st.divider(); st.success(f"Tiket {ld['ID']} Telah Dibuka!"); c1, c2, c3 = st.columns(3)
        ...
        if ld['Email']:
            if c3.button("📧 Email Auto"):
                ...
                if ok: st.toast(m, icon='✅'); else: st.error(m)

# === PAGE: JUALAN KEDAI (V72 - INTEGRATED STOCK) ===
elif st.session_state.page == "🛒 JUALAN KEDAI":
    st.title("🛒 Sistem Jualan (POS)")
    df_m = load_data("Master_Inventory")
    
    # Create Item List for Dropdown (Filter Stock > 0)
    stock_options = {}
    if not df_m.empty:
    ...
    with tab_pos:
        ...
        if st.session_state.pos_cart:
            st.divider(); st.subheader("🛍️ Bakul Jualan"); st.markdown("---")
            for i, row in enumerate(st.session_state.pos_cart):
                c1, c2, c3, c4 = st.columns([3, 1, 1, 0.5])
                c1.write(f"**{row['Item']}**"); c2.write(f"x {row['Qty']}"); c3.write(f"RM {row['Total']:.2f}")
                if c4.button("❌", key=f"del_{i}"): st.session_state.pos_cart.pop(i); st.rerun()
            st.markdown("---"); grand_total = sum([x['Total'] for x in st.session_state.pos_cart])
            ...
            with st.form("checkout_form"):
                ...
                if st.form_submit_button("✅ Bayar & Cetak Resit", use_container_width=True):
                    sid = f"SALE-{datetime.now().strftime('%d%H%M')}"; tgl = datetime.now().strftime("%Y-%m-%d")
                    ...
                    for row in st.session_state.pos_cart:
                        ...
                        # DEDUCT STOCK LOGIC
                        if 'ItemCode' in row: update_stock(row['ItemCode'], -row['Qty'])
                            
                    st.session_state.last_sale = {"ID": sid, "Tarikh": tgl, "Customer": cust_name, "Total": grand_total, "Items_List": st.session_state.pos_cart}
                    st.session_state.pos_cart = []; st.toast("Transaksi Berjaya! Stok Dikemaskini.", icon='💰'); time.sleep(1); st.rerun()
            
            if st.button("❌ Kosongkan Semua Bakul"): st.session_state.pos_cart = []; st.rerun()
    ...
    with tab_manage:
        df_s = load_data("Sales"); st.dataframe(df_s, use_container_width=True)

# === PAGE: UPDATE STATUS (V72 - INTEGRATED PARTS) ===
elif st.session_state.page == "🔧 UPDATE STATUS":
    st.title("🔧 Bilik Technician")
    df = load_data("Tickets"); df_m = load_data("Master_Inventory")
    ...
    if not df.empty:
        ...
        with st.expander("ℹ️ MAKLUMAT TIKET & KOMUNIKASI (KLIK EDIT)", expanded=True):
            ...
            with st.expander("🖨️ GENERATE QR STICKER (HEALTH CARD)"):
                ...
                if app_url and job.get('SN'):
                    ...
                    qr = qrcode.QRCode(box_size=10, border=2); qr.add_data(qr_data); qr.make(fit=True)
                    img_qr = qr.make_image(fill_color="black", back_color="white")
                    buffer = BytesIO(); img_qr.save(buffer, format="PNG"); img_bytes = buffer.getvalue()
                    c_qr1, c_qr2 = st.columns([1, 2]); c_qr1.image(img_bytes, caption=f"S/N: {job.get('SN')}", width=150)
                    c_qr2.write(f"**URL:** {qr_data}"); c_qr2.info("👉 Right-click gambar QR > 'Save Image' untuk print.")

            st.divider(); ca, cb = st.columns(2); wa_url = generate_links("WA", job)
        ...
        with c_r:
            ...
            if not df_p.empty and 'TicketID' in df_p.columns: parts = df_p[df_p['TicketID'].astype(str) == str(pid)]
            else: parts = pd.DataFrame()
            ...
            with t1:
                ...
                with st.form("upd_status"):
                    ...
                    if st.form_submit_button("UPDATE"):
                        sheet = get_client().open_by_key(SHEET_ID).worksheet("Tickets"); cl = robust_api_call(sheet.find, str(pid))
                        if cl:
                            robust_api_call(sheet.update_cell, cl.row, 12, stt); robust_api_call(sheet.update_cell, cl.row, 13, kos)
                            robust_api_call(sheet.update_cell, cl.row, 14, hj); robust_api_call(sheet.update_cell, cl.row, 16, nt)
                            st.cache_data.clear(); st.toast("Status Dikemaskini!", icon='🎉'); time.sleep(1); st.rerun()
            ...
            with t2:
                ...
                if not parts.empty:
                    ...
                    if d != "-" and st.button("Hapus Part"): delete_part(d); st.toast("Part dihapus!", icon='🗑️'); time.sleep(1); st.rerun()
                else: st.info("Tiada part digunakan.")
                ...
                with st.form("use_part_form", clear_on_submit=True):
                    ...
                    if st.form_submit_button("Guna Part Ini"):
                        if sel_part != "-":
                            ...
                            # Add to Parts Log
                            exp = (datetime.now() + pd.DateOffset(months=3)).strftime("%Y-%m-%d") # Default 3 bulan warranty
                            add_row("Parts", [f"P-{int(time.time())}", pid, item_data['ItemName'], item_data['Supplier'], str(datetime.now().date()), 3, exp, item_data['CostPrice']])
                            # Deduct Stock
                            update_stock(item_data['ItemCode'], -1)
                            st.toast(f"{item_data['ItemName']} ditambah ke Job!", icon='✅'); time.sleep(1); st.rerun()
                        else: st.warning("Pilih part dulu.")

# === PAGE: PENGURUSAN STOK (V72 NEW) ===
    ...
    # 1. RESTOCK FORM
    with t_restock:
        st.subheader("Beli Barang Dari Supplier")
        with st.form("restock_form", clear_on_submit=True):
            c1, c2 = st.columns(2)
            inv_no = c1.text_input("No Invoice / Resit")
            supp = c2.text_input("Nama Supplier")
            
            st.markdown("---")
            c3, c4 = st.columns(2)
            # Logic: New Item vs Existing Item
            df_m = load_data("Master_Inventory")
            existing_items = df_m['ItemName'].tolist() if not df_m.empty else []
            
            item_mode = st.radio("Jenis Barang", ["Barang Baru (First Time)", "Restock Barang Lama"], horizontal=True)
            
            if item_mode == "Restock Barang Lama":
                item_name = st.selectbox("Pilih Barang", existing_items)
                cat = "Auto"
            else:
                item_name = st.text_input("Nama Barang Baru")
                cat = st.selectbox("Kategori", ["Sparepart", "Aksesori", "Tools"])
            
            c5, c6, c7 = st.columns(3)
            qty = c5.number_input("Kuantiti", 1, 1000, 1)
            cost = c6.number_input("Harga Kos Seunit (RM)", 0.0)
            sell = c7.number_input("Harga Jual Seunit (RM)", 0.0)
            warranty = st.number_input("Tempoh Warranty (Hari) - 0 jika tiada", 0)
            
            if st.form_submit_button("✅ Simpan Stok"):
                if item_name and cost > 0:
                    tgl = datetime.now().strftime("%Y-%m-%d")
                    
                    # 1. Generate Item Code (Unique)
                    if item_mode == "Barang Baru (First Time)":
                        code = f"ITM-{int(time.time())}"
                        # Add to Master Inventory
                        add_row("Master_Inventory", [code, item_name, cat, cost, sell, qty, warranty, supp, tgl])
                    else:
                        # Find Code from existing
                        row_data = df_m[df_m['ItemName'] == item_name].iloc[0]
                        code = row_data['ItemCode']
                        # Update Master (Add Qty, Update Prices)
                        update_stock(code, qty)
                        # Optional: Update Cost/Sell price logic here if needed (Skipped for simplicity, assumes manual update in Master List)
                    
                    # 2. Add to Restock Log
                    log_id = f"LOG-{int(time.time())}"
                    total_cost = qty * cost
                    add_row("Restock_Log", [log_id, tgl, inv_no, supp, code, item_name, qty, cost, total_cost])
                    
                    st.success(f"Stok {item_name} berjaya ditambah! ({qty} unit)")
                    time.sleep(1); st.rerun()
                else:
                    st.warning("Sila isi nama barang dan harga.")
    ...
    with t_master:
        ...
        if not df_m.empty:
            ...
            st.info("Nota: Untuk edit harga/nama, sila edit terus di Google Sheet 'Master_Inventory' buat masa ini.")
        else:
            st.info("Belum ada stok. Sila masuk stok dulu.")
...
# === PAGE: HISTORY DEVICE ===
    ...
    if search_sn:
        ...
        if not df.empty:
            ...
            else: st.warning("Tiada rekod dijumpai untuk S/N ini.")
        else: st.error("Database kosong.")
...
# === PAGE: LAPORAN ===
    ...
    if not df.empty:
        ...
        st.divider(); st.subheader("📅 Prestasi Berkala")
...
# === PAGE: TETAPAN (V69: DYNAMIC OPTIONS) ===
