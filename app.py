...
if 'config' not in st.session_state:
    st.session_state.config = {
        ...
        # Pilihan Default (Boleh ubah di Tetapan)
...
def init_db():
    ...
    try:
        ...
        if len(ws_s.row_values(1)) != len(h_s): ws_s.update("A1:I1", [h_s])
        ...
        # 5. Master Inventory (Gudang Utama - V72)
        ...
        # 6. Restock Log (Rekod Beli Barang - V72)
        ...
        if df.empty:
            if tab_name == "Tickets":
                df = pd.DataFrame(columns=["ID", "Tarikh", "Customer", "Phone", "Email", "Model", "SN", "Password", "Masalah", "Fizikal", "Aksesori", "Status", "Kos_Part", "Harga_Jual", "Image_Link", "Tech_Note"])
                 ...
                 df = pd.DataFrame(columns=["ItemCode", "ItemName", "Category", "CostPrice", "SellPrice", "CurrentStock", "WarrantyDays", "Supplier", "LastUpdated"])
...
def generate_pdf(t, type="SERVICE"):
    ...
    if 'Model' in t: 
        ...
        for itm in items:
            ...
            y_item = draw_wrapped_text(p, f"{str(itm.get('Item', '-'))} (W: {itm.get('Warranty','-')})", 50, y, 280); y = y_item - 10
        p.line(40, y, w-40, y)
        ...
        if not history.empty:
            ...
            for _, row in history.iloc[::-1].iterrows():
                with st.expander(f"📅 {row['Tarikh']} : {row['Masalah']}"):
                    st.markdown(f"""<div style="background-color: #d4edda; padding: 10px; border-radius: 5px; border: 1px solid #c3e6cb; color: #155724;"><strong>✅ Solution / Tindakan:</strong><br>{row['Tech_Note']}</div>""", unsafe_allow_html=True)
            st.divider(); st.caption(f"Profil ini dijana automatik oleh sistem {cfg.get('Company_Name')}")
...
# === PAGE: JUALAN KEDAI (V73 - WARRANTY ADDED) ===
    ...
    with tab_pos:
        with st.container(border=True):
            ...
            with st.form("add_item_form", clear_on_submit=True):
                ...
                # V73: WARRANTY SELECTION FOR CLIENT
                    ...
                    if selected_item_label != "-":
                        ...
                        if qty <= current_stock:
                            ...
                            st.session_state.pos_cart.append({
                                ...
                                "Harga_Unit": price,
                                "Total": qty * price,
                                "Warranty": warr  # Save warranty info
                            })
                            st.toast(f"{item_data['ItemName']} ditambah!", icon='🛒')
                        else: st.error(f"Stok tak cukup! Tinggal {current_stock} unit.")
                    else: st.warning("Sila pilih barang.")

        if st.session_state.pos_cart:
            st.divider(); st.subheader("🛍️ Bakul Jualan"); st.markdown("---")
            for i, row in enumerate(st.session_state.pos_cart):
                c1, c2, c3, c4 = st.columns([3, 1, 1, 0.5])
                c1.write(f"**{row['Item']}** ({row['Warranty']})"); c2.write(f"x {row['Qty']}"); c3.write(f"RM {row['Total']:.2f}")
            ...
            with st.form("checkout_form"):
                ...
                if st.form_submit_button("✅ Bayar & Cetak Resit", use_container_width=True):
                    ...
                    for row in st.session_state.pos_cart:
                        # V73: Save Warranty to Sales Record
...
# === PAGE: UPDATE STATUS (V73 - WARRANTY ADDED) ===
        ...
        with c_r:
            ...
            with t2:
                ...
                with st.form("use_part_form", clear_on_submit=True):
                    ...
                    # V73: WARRANTY SELECTION FOR CLIENT (IN REPAIR)
                    ...
                    if st.form_submit_button("Guna Part Ini"):
                        if sel_part != "-":
                            ...
                            # Calculate Expire Date based on selection (1 or 3 months)
                            ...
                            st.toast(f"{item_data['ItemName']} ditambah ke Job (Warranty {warr_part} Bulan)!", icon='✅'); time.sleep(1); st.rerun()
...
# === PAGE: PENGURUSAN STOK ===
    ...
    with t_restock:
        ...
        with st.form("restock_form", clear_on_submit=True):
            c1, c2 = st.columns(2); inv_no = c1.text_input("No Invoice / Resit"); supp = c2.text_input("Nama Supplier")
            st.markdown("---"); c3, c4 = st.columns(2)
            df_m = load_data("Master_Inventory"); existing_items = df_m['ItemName'].tolist() if not df_m.empty else []
            ...
            if item_mode == "Restock Barang Lama":
                item_name = st.selectbox("Pilih Barang", existing_items); cat = "Auto"
            else:
                item_name = st.text_input("Nama Barang Baru"); cat = st.selectbox("Kategori", ["Sparepart", "Aksesori", "Tools"])
            c5, c6, c7 = st.columns(3); qty = c5.number_input("Kuantiti", 1, 1000, 1); cost = c6.number_input("Harga Kos Seunit (RM)", 0.0); sell = c7.number_input("Harga Jual Seunit (RM)", 0.0)
            warranty = st.number_input("Tempoh Warranty Supplier (Hari) - Rujukan Sahaja", 0)
            ...
            if st.form_submit_button("✅ Simpan Stok"):
                if item_name and cost > 0:
                    ...
                    if item_mode == "Barang Baru (First Time)":
                        ...
                        row_data = df_m[df_m['ItemName'] == item_name].iloc[0]; code = row_data['ItemCode']
                        update_stock(code, qty)
                    log_id = f"LOG-{int(time.time())}"; total_cost = qty * cost
                    add_row("Restock_Log", [log_id, tgl, inv_no, supp, code, item_name, qty, cost, total_cost])
                    st.success(f"Stok {item_name} berjaya ditambah! ({qty} unit)"); time.sleep(1); st.rerun()
                else: st.warning("Sila isi nama barang dan harga.")

    with t_master:
        df_m = load_data("Master_Inventory")
        if not df_m.empty: st.dataframe(df_m, use_container_width=True); st.info("Nota: Untuk edit harga/nama, sila edit terus di Google Sheet 'Master_Inventory' buat masa ini.")
        else: st.info("Belum ada stok. Sila masuk stok dulu.")

    with t_log:
        df_r = load_data("Restock_Log")
        if not df_r.empty: st.dataframe(df_r, use_container_width=True)
        else: st.info("Tiada rekod pembelian.")
    ...
    if search_sn:
        ...
        if not df.empty:
            ...
            if not history.empty:
                ...
                st.write("---"); st.write("### 📜 Butiran Terperinci")
                for _, row in history.iterrows():
                    with st.expander(f"{row['Tarikh']} - {row['Masalah']} ({row['Status']})"):
                        st.write(f"**ID Tiket:** {row['ID']}"); st.write(f"**Customer:** {row['Customer']}")
...
# === PAGE: LAPORAN ===
    ...
    if not df.empty:
        ...
        df['Harga_Jual'] = df['Harga_Jual'].apply(safe_float); df['Kos_Part'] = df['Kos_Part'].apply(safe_float)
        df['Untung'] = df['Harga_Jual'] - df['Kos_Part']
        serv_sales = df['Harga_Jual'].sum(); shop_sales = df_s['Total'].apply(safe_float).sum() if not df_s.empty else 0
        total_rev = serv_sales + shop_sales
        m1, m2, m3 = st.columns(3)
        m1.metric("Total Revenue", f"RM {total_rev:.2f}"); m2.metric("Untung Servis", f"RM {df['Untung'].sum():.2f}"); m3.metric("Jualan Kedai", f"RM {shop_sales:.2f}")
