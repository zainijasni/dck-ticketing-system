...
def generate_pdf(t, type="SERVICE"):
    ...
    y = h-250
    p.setFont("Helvetica-Bold", 11); p.setFillColorRGB(0.9, 0.9, 0.9); p.rect(40, y, w-80, 20, fill=1); p.setFillColorRGB(0, 0, 0)
    ...
    if type in ["INVOICE", "SALES"] or t.get('Status') in ['Done', 'Collected']:
        ...
        y -= 30; p.setFont("Helvetica-Bold", 14); p.drawRightString(w-50, y, f"TOTAL: RM {total:.2f}")
...
# =============================================================================
# 🚦 V69 GATEKEEPER LOGIC: DIGITAL HEALTH CARD (IMPROVED UI)
# =============================================================================
    ...
    if not df.empty:
        ...
        if not history.empty:
            ...
            with st.container(border=True):
                ...
                st.markdown("---")
                st.write("**Sejarah Isu & Pembaikan (Terkini):**")
            ...
            if len(history) > 1:
                st.write("### 📜 Sejarah Terdahulu (Klik untuk lihat Solution)")
                # Reverse order (Newest first), skip the first one (latest)
                prev_jobs = history.iloc[:-1].iloc[::-1]
                
                for _, row in prev_jobs.iterrows():
                    # V69 CHANGE: Clickable Expander for History
...
# === PAGE: DAFTAR TIKET ===
    ...
    with st.form("reg_ticket", clear_on_submit=True):
        ...
        with c3:
            ...
            fiz = st.multiselect("Fizikal", opt_fiz); acc = st.multiselect("Aksesori", opt_acc)
        ...
        if st.session_state.pos_cart:
            ...
            with st.form("checkout_form"):
                ...
                if st.form_submit_button("✅ Bayar & Cetak Resit", use_container_width=True):
                    ...
                    st.session_state.pos_cart = []; st.toast("Transaksi Berjaya!", icon='💰'); time.sleep(1); st.rerun()
...
# === PAGE: TETAPAN (V69: DYNAMIC OPTIONS) ===
    ...
    with st.form("conf"):
        ...
        opt_m = st.text_area("Senarai Masalah", st.session_state.config.get("Options_Masalah", "Slow, Screen, Battery"))
        opt_f = st.text_area("Senarai Fizikal", st.session_state.config.get("Options_Fizikal", "Calar, Pecah, Sempurna"))
        opt_a = st.text_area("Senarai Aksesori", st.session_state.config.get("Options_Aksesori", "Bag, Charger"))
